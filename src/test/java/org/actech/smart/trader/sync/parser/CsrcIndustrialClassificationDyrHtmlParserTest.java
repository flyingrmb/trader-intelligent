...
@ActiveProfiles("dev")
public class CsrcIndustrialClassificationDyrHtmlParserTest {
    @Autowired
    private CsrcIndustrialClassificationDyrHtmlParser parser;
    ...
    public void trueIfDocumentContainsContent() throws Exception {
        ...
        Resource resource = resourceLoader.getResource("classpath:证监会行业股息率.htm");
    ...
    public void falseIfDocumentContainsNoContent() throws Exception {
        ResourceLoader resourceLoader = new DefaultResourceLoader();
        Resource resource = resourceLoader.getResource("classpath:证监会行业股息率空.htm");
        Document document = Jsoup.parse(resource.getFile(), "UTF-8");
