...
@ActiveProfiles("dev")
public class CsrcIndustrialClassificationTtmHtmlParserTest {
    @Autowired
    private CsrcIndustrialClassificationTtmHtmlParser parser;
    ...
    public void trueIfDocumentContainsContent() throws Exception {
        ...
        Resource resource = resourceLoader.getResource("classpath:证监会行业滚动市盈率.htm");
    ...
    public void falseIfDocumentContainsNoContent() throws Exception {
        ResourceLoader resourceLoader = new DefaultResourceLoader();
        Resource resource = resourceLoader.getResource("classpath:证监会行业滚动市盈率空.htm");
        Document document = Jsoup.parse(resource.getFile(), "UTF-8");
